...
class SSTIDetectionSkill(SecuritySkill):
    def __init__(self) -> None:
        super().__init__()
        self._capabilities = [SkillCapability.SSTI]

    def _create_metadata(self) -> SkillMetadata:
        return SkillMetadata.create(
            skill_id="ssti_detection",
            name="SSTI Detection",
            description="Identifies server-side template injection vulnerabilities in template engines",
            version="1.0.0",
            author="HunterX",
    ...
    def _on_execute(self, target: str, context: Optional[SkillContext] = None) -> SkillResult:
        return SkillResult.success(
            ...
            confidence=0.7,
            findings=[{"type": "ssti_check", "detail": f"Scanned for SSTI on {target}"}],

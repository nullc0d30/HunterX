...
class KubernetesDetectionSkill(SecuritySkill):
    def __init__(self) -> None:
        super().__init__()
        self._capabilities = [SkillCapability.KUBERNETES]

    def _create_metadata(self) -> SkillMetadata:
        return SkillMetadata.create(
            skill_id="kubernetes_detection",
            name="Kubernetes Detection",
            description="Detects Kubernetes API servers, dashboard exposures, and misconfigured RBAC",
            version="1.0.0",
            author="HunterX",
    ...
    def _on_execute(self, target: str, context: Optional[SkillContext] = None) -> SkillResult:
        return SkillResult.success(
            ...
            confidence=0.7,
            findings=[{"type": "k8s_detection", "detail": f"Probed for Kubernetes services on {target}"}],

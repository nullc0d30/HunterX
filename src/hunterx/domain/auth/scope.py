...
"""Scope enforcement for authentication intelligence.
...
Discovered authentication intelligence is never used to silently expand scope:
the enforcer authorizes the *asset* being analysed, not the authentication
surfaces or identity providers found on it.
"""
...
class AuthScopePolicy:
    """The authentication intelligence scope policy for a mission.
...
class AuthScopeEnforcer:
    """Evaluate assets and observations against an auth scope policy."""

    def __init__(self, policy: AuthScopePolicy | None = None) -> None:
        self._policy = policy or AuthScopePolicy()
    ...
    @property
    def policy(self) -> AuthScopePolicy:
    ...
    def allows_target(self, target: AuthTarget) -> ScopeDecision:
        """Return whether a whole auth target is in scope."""
    ...
    def allows_observation(self, observation: Any) -> ScopeDecision:
        ...
        surface endpoint on an external identity-provider host is never used to
        expand scope.
        """

...
"""Scope enforcement for technology fingerprinting.

Decides whether a hostname, domain, IP or URL may be fingerprinted or persisted
...
Discovered technology intelligence is never used to silently expand scope: the
enforcer authorizes the *asset* being fingerprinted, not the technologies found
on it.
"""
...
class TechnologyScopePolicy:
    """The fingerprinting scope policy for a mission.

    Attributes:
        roots: authorized root domains (e.g. ``example.com``). A hostname is in
            scope when it is a root or a descendant of a root.
        root_cidrs: authorized CIDRs whose addresses may be fingerprinted.
...
class TechnologyScopeEnforcer:
    """Evaluate assets against a fingerprinting scope policy."""

    def __init__(self, policy: TechnologyScopePolicy | None = None) -> None:
        self._policy = policy or TechnologyScopePolicy()
    ...
    def allows_target(self, target: TechTarget) -> ScopeDecision:
        """Return whether a whole fingerprinting target is in scope."""
    ...
    def allows_observation(self, observation: TechnologyObservation) -> ScopeDecision:
        """Return whether an observation may be persisted."""
        return self.allows_asset(observation.asset, observation.asset_type)

    def filter_observations(self, observations: Iterable[TechnologyObservation]) -> list[TechnologyObservation]:
...
def _url_host(url: str) -> str:
    """Return the hostname of a URL (empty when unparseable)."""

...
"""Base adapter for Live Host & Service Discovery tools.

Every live discovery tool adapter implements the SDK :class:`ToolAdapter`
...
captured output into canonical discovery observations (:class:`LiveHost`,
:class:`PortFinding`, :class:`ServiceFinding`, :class:`TlsFinding`,
:class:`HttpFinding`) and serialises them into the pipeline's JSON payload
under the ``observations`` key with a ``type`` discriminator. Adapters stay
binary-free in unit tests: tests inject a fake runner and assert on the JSON
payload.

The runner is imported from ``hunterx.tools.recon.runner`` — the single guarded
subprocess seam — so live discovery adapters never touch ``subprocess``
directly.
"""
...
from hunterx.domain.livehost.models import observations_from_payload
...
class LiveToolAdapter(ToolAdapter, abc.ABC):
    """Shared base for SDK live discovery adapters.
    ...
    def prepare(self, context: ExecutionContext) -> None:
        """No setup required for live discovery tools; hook kept for parity."""
    ...
    def run(self, context: ExecutionContext, collector: OutputCollector) -> None:
        """Invoke the binary, capture output and emit discovery observations."""
    ...
    def normalize(self, context: ExecutionContext, output: ExecutionOutput) -> ToolOutput:
        """Project discovery observations into the legacy asset surface."""
        tool_output = ToolOutput()
        if output.stdout:
            tool_output.raw = output.stdout
        _hosts, _ports, _services, _tls, _http = observations_from_payload(output.json)
        observations: list[Any] = [*_hosts, *_ports, *_services, *_tls, *_http]
    ...
    @staticmethod
    def _payload(observations: Sequence[Any]) -> dict[str, Any]:
        """Build the JSON payload attached to the execution output."""
        return {
            "observations": [observation.to_dict() for observation in observations if hasattr(observation, "to_dict")],
    ...
    def _param_ports(self, context: ExecutionContext, default: tuple[int, ...] = ()) -> tuple[int, ...]:
        """Return the ordered, de-duplicated ports requested for an execution."""
        value = self._param(context, "ports")
        if isinstance(value, (list, tuple)) and value:
            ...
            return tuple(ports)
        if default:
            return tuple(dict.fromkeys(default))
        return ()

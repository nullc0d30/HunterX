...
class PayloadAgent(SecurityAgent):
    def __init__(self, agent_id: str = ""):
        super().__init__(agent_id=agent_id, name="PayloadAgent", version="1.0.0")
        self._capabilities = [AgentCapability.PAYLOAD_SELECTION]
    ...
    def _on_create_goals(self, objective: str, context: Dict[str, Any]) -> List[Goal]:
        return [
            Goal.create(
                goal_type=GoalType.PAYLOAD_SELECTION,
                objective=f"Payload selection: {objective}",
                context=context,
